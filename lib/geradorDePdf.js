...
module.exports = function(danfe, args, callback) {
    ...
    function desenharPagina() {
        ...
        negrito(danfe.Emitente.Nome, identificacaoDoEmitenteX, identificacaoDoEmitenteY + args.ajusteYDaIdentificacaoDoEmitente, identificacaoDoEmitenteLargura, 'center', 8 + identificacaoDoEmitenteFonte);

        if(danfe.Emitente.Endereco.Logradouro) {
            normal(danfe.Emitente.Endereco.Logradouro,
        ...
        if(danfe.Emitente.Telefone) {
            ...
            normal('Telefone: ' + danfe.Emitente.Telefone,
                identificacaoDoEmitenteX,
                pdf.y - margemTopo + 2,
            identificacaoDoEmitenteLargura, 'center', 6 + identificacaoDoEmitenteFonte);
        }
        ...
        if(danfe.Emitente.Email) {
            normal('Email: ' + danfe.Emitente.Email,
                identificacaoDoEmitenteX,
        ...
        normal(danfe.InformacoesComplementares, 1, alturaInicialDoSetimoBloco + 7.5, 386, 'justify', 6);
        ...
        campo(danfe.Destinatario.Endereco.Logradouro, 1.5, 230, 272, 'left', args.tamanhoDaFonteDoCampo - 0.5);
        ...
        campo(danfe.Transportador.Endereco.Logradouro, 1.5, 349, 254, 'left', args.tamanhoDaFonteDoCampo - 0.5);
    ...
    var numeroDePaginas = 1,
        ...
        function renderizarLinha(pdf) {
            normal(item.Codigo, 1.5, yDoItemAtual, 51, 'center', args.tamanhoDaFonteDosItens, pdf);
            var maiorY = pdf.y;
            ...
            normal(item.NcmSh, 235.5, yDoItemAtual, 32.5, 'center', args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);

            normal(item.OCst, 270, yDoItemAtual, 21, 'center', args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);

            normal(item.Cfop, 293.5, yDoItemAtual, 21, 'center', args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);

            normal(item.Unidade, 315, yDoItemAtual, 16.5, 'center', args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);

            normal(item.Quantidade, 332.5, yDoItemAtual, 37.5, 'right', args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);

            normal(item.ValorUnitario, 372, yDoItemAtual, 32.5, 'right', args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);

            normal(item.ValorTotal, 407, yDoItemAtual, 31, 'right', args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);

            normal(item.BaseDeCalculoDoIcms, 440, yDoItemAtual, 32.5, 'right', args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);

            normal(item.BaseDeCalculoDoIcms, 440, yDoItemAtual, 32.5, 'right', args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);

            normal(item.ValorDoIcms, 474.5, yDoItemAtual, 32, 'right', args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);

            normal(item.ValorDoIpi, 508.5, yDoItemAtual, 26, 'right', args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);

            normal(item.AliquotaDoIcms, 536, yDoItemAtual + 0.65, 19, 'center', args.tamanhoDaFonteDosItens - 1, pdf);
            maiorY = Math.max(maiorY, pdf.y);

            normal(item.AliquotaDoIpiFormatada, 556.5, yDoItemAtual, 29, 'center', args.tamanhoDaFonteDosItens, pdf);
            maiorY = Math.max(maiorY, pdf.y);
